...
export default function GroupChat({ user, profile }: { user: any; profile?: any }) {
  ...
  useEffect(() => {
    // Fetch user profile for chat access and role check
    const fetchProfile = async () => {
      if (!user?.id) return;
      const { data } = await supabase.from('profiles').select('chat_access, role, csc_role').eq('id', user.id).single();
      const p = data || profile;
      if (p) {
        const isSuperAdmin = p.role === 'Admin' || p.csc_role === 'Admin';
        const hasCsc = (p.csc_role || 'Executive') !== 'None';
        const allowedToSee = isSuperAdmin || hasCsc;
        setIsVisible(allowedToSee);
        setCanChat(p.role === 'Viewer' || !allowedToSee ? false : (p.chat_access ?? true));
      } else {
