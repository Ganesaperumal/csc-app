...
export default function ClosedJobsPage() {
  ...
  useEffect(() => {
    ...
    try {
      sessionStorage.setItem('csc_last_jobs_page', '/home/closed-jobs');
    } catch {}
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'jobs' }, (payload: any) => {
        if (payload.eventType === 'UPDATE' && payload.new?.job_number) {
          ...
          jobsCache.patchItem('closed-jobs-raw-data', payload.new);
        } else if (payload.eventType === 'INSERT' && payload.new?.job_number) {
          jobsCache.invalidate('closed-jobs-raw-data');
          ...
          jobsCache.removeItem('closed-jobs-raw-data', payload.old.job_number);
        } else {
  ...
  const fetchJobs = async (force = false) => {
    ...
    try {
      ...
      // Check cache first
      const cached = !force ? jobsCache.get<any[]>('closed-jobs-raw-data') : null;
      if (cached && cached.length > 0) {
        ...
        jobsCache.set('closed-jobs-raw-data', data);
      }
  ...
  const SEARCH_FIELDS = ['job_number', 'enq_number', 'customer_name', 'company', 'branch', 'origin', 'destination',
    'goods_type', 'goods_track_status', 'erp_status', 'spoc_name', 'invoice_number', 'customer_phone', 'erp_job_id'];
  ...
  return (
    <div>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
            ...
            <input
              ...
              style={{
                paddingLeft: '2.5rem', paddingRight: globalSearch ? '2.5rem' : '1rem',
                paddingTop: '0.55rem', paddingBottom: '0.55rem', width: '260px',
                borderRadius: '99px', border: '1.5px solid var(--border-color)',
                background: 'var(--surface-color)', color: 'var(--text-primary)',
                fontSize: '0.85rem', outline: 'none', transition: 'all 0.2s ease',
            ...
            />
            {globalSearch && (
              <button onClick={() => setGlobalSearch('')} style={{ position: 'absolute', right: '0.75rem', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-secondary)', display: 'flex', padding: 0 }}>
      ...
      <div className={`glass ${styles.tableContainer}`} style={{ height: 'calc(100vh - 114px)' }}>
        {loading ? (
          ...
          <table className={styles.table}>
            ...
            <tbody>
              {sortedJobs.length === 0 ? (
                <tr><td colSpan={visibleColumns.length} className={styles.noData}>No closed jobs found.</td></tr>
              ) : (
                sortedJobs.map(job => (
                  <tr
                    key={job.job_number}
                    onClick={() => router.push(`/home/job/${encodeURIComponent(job.job_number)}`)}
                    style={{ cursor: 'pointer', opacity: job.erp_status?.toLowerCase() === 'canceled' || job.erp_status?.toLowerCase() === 'cancelled' ? 0.75 : 1 }}
                  >
                    {orderedColumns.filter(id => visibleColumns.includes(id) && ALL_COLUMNS.some(c => c.id === id)).map(colId => {
