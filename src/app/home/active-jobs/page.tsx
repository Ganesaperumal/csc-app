...
function JobsTable() {
  ...
  // Screen-size based batch pagination
  const [visibleCount, setVisibleCount] = useState(() => {
    if (typeof window !== 'undefined') {
      return Math.max(40, Math.ceil((window.innerHeight - 240) / 40) * 2);
    }
    return 40;
  });
  ...
  useEffect(() => {
    ...
    try {
      sessionStorage.setItem('csc_last_jobs_page', '/home/active-jobs');
    } catch {}
  }, []);
  ...
  // Global text search — same fields as Ctrl+K command palette
  ...
  const SEARCH_FIELDS = ['job_number', 'enq_number', 'customer_name', 'company', 'branch', 'origin', 'destination',
    'goods_type', 'goods_track_status', 'car_track_status', 'erp_status', 'spoc_name',
    'packing_team_supervisor', 'invoice_number', 'customer_phone', 'erp_job_id'];
  ...
  return (
    <div className="page-enter">
        ...
        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
            ...
            <input
              ...
              style={{
                paddingLeft: '2.5rem',
                paddingRight: globalSearch ? '2.5rem' : '1rem',
                paddingTop: '0.55rem',
          ...
          >
            <button 
              ...
              style={{ 
                ...
                borderRadius: '99px',
                border: '1.5px solid var(--border-color)',
                background: notifications.length > 0 ? 'linear-gradient(135deg, #ef4444, #dc2626)' : 'var(--surface-color)', 
                color: notifications.length > 0 ? '#fff' : 'var(--text-primary)',
                cursor: 'pointer',
            ...
            >
              ...
              {notifications.length > 0 && <span style={{ fontWeight: 'bold' }}>{notifications.length}</span>}
            </button>
                    ...
                    notifications.map(n => {
                      ...
                      return (
                        <div key={n.id} style={{ 
                          ...
                          setShowNotifications(false);
                          router.push('/home/job/' + encodeURIComponent(n.job_number));
                        }}
                        onMouseEnter={(e) => {
      ...
      <div ref={tableContainerRef} onScroll={handleTableScroll} className={`glass ${styles.tableContainer}`} style={{ height: 'calc(100vh - 238px)' }}>
        {loading ? (
