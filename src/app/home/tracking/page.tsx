...
export default function SpocPortalPage() {
  ...
  const checkAccess = async (userId: string) => {
    const { data: profile } = await supabase.from('profiles').select('role, tracking_role').eq('id', userId).single();
    // Access allowed if user has any non-None tracking role OR is Admin
    if (profile && (profile.role === 'Admin' || (profile.tracking_role && profile.tracking_role !== 'None'))) {
      setIsLoggedIn(true);
    } else {
      router.push('/home');
    }
