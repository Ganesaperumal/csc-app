...
export default function MissingDataSection() {
  ...
  const fetchData = async () => {
    ...
    if (error) {
      showToast(`Error loading missing data jobs: ${error.message}`, 'error');
      setLoading(false);
      return;
    }
  ...
  const sectionHeader = (title: string, count: number, color: string) => (
    <div style={{ marginBottom: '0.75rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
        <h4 style={{ margin: 0, fontSize: '0.95rem', fontWeight: 800, color }}>{title}</h4>
        <span style={{
  ...
  if (loading) {
    return (
      <div style={{ padding: '1rem', display: 'flex', alignItems: 'center', gap: '0.75rem', color: 'var(--text-secondary)', fontSize: '0.85rem' }}>
        <div style={{ width: '18px', height: '18px', border: '2px solid var(--border-color)', borderTopColor: '#f59e0b', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
        <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
        Loading missing data jobs…
      </div>
    );
  }

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.75rem', marginBottom: '2rem', paddingBottom: '1.5rem', borderBottom: '1px solid rgba(148, 163, 184, 0.2)' }}>
      
      {/* ── Section 1: Missing Quote Value ── */}
      <div>
        {sectionHeader('1. Missing Quote Value', quoteJobs.length, '#f59e0b')}
        {quoteJobs.length === 0 ? (
          <div style={{ padding: '1rem 1.25rem', background: 'rgba(16, 185, 129, 0.08)', borderRadius: '10px', border: '1px solid rgba(16, 185, 129, 0.2)', color: '#10b981', fontWeight: 700, fontSize: '0.84rem' }}>
            ✅ All unbilled jobs have a quote value assigned — nothing to fill in!
          </div>
        ) : (
          <div style={{ border: '1px solid var(--border-color)', borderRadius: '12px', overflow: 'hidden', maxHeight: '380px', overflowY: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead style={{ background: 'var(--surface-color)', position: 'sticky', top: 0, zIndex: 2 }}>
                <tr>
      ...
      <div>
        {sectionHeader('2. Missing Unbilled SPOC', spocJobs.length, '#6366f1')}
        {spocJobs.length === 0 ? (
          <div style={{ padding: '1rem 1.25rem', background: 'rgba(16, 185, 129, 0.08)', borderRadius: '10px', border: '1px solid rgba(16, 185, 129, 0.2)', color: '#10b981', fontWeight: 700, fontSize: '0.84rem' }}>
          ...
          <div style={{ border: '1px solid var(--border-color)', borderRadius: '12px', overflow: 'hidden', maxHeight: '380px', overflowY: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead style={{ background: 'var(--surface-color)', position: 'sticky', top: 0, zIndex: 2 }}>
                <tr>
                  <th style={thStyle}>Enquiry No.</th>
                  <th style={thStyle}>Company / Customer</th>
